...
# попытка регулировать плотность огня
# регулирую время жизни ракеты и расстояние точки старта ракеты от корабля
...
class Ship(games.Sprite):
    ...
    def __init__(self, image, x, y):
        ...
        self.interval = Ship.FIRE_INTERVAL
    ...
    def update(self):
        ...
        # регулируем плотность огня
        if self.interval > 0:
            self.interval -= 1
        # если нажат пробел, выпустить ракету
        if games.keyboard.is_pressed(games.K_SPACE) and self.interval == 0:
            self.interval = Ship.FIRE_INTERVAL
