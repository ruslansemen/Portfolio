...
class Ball:
    ...
    def __init__(self, canvas, paddle, textend, score, color):
        self.canvas = canvas
        self.paddle = paddle
...
class Score:
    def __init__(self, canvas):
        self.canvass = canvas
        self.id = canvas.create_text(
            460, 15, text='Счёт: 0', font=('Times', 15))
        self.string_score = ''
        self.num_score = 0

    def increase(self):
        self.num_score += 1
        self.string_score = 'Счёт: ' + str(self.num_score)
        self.canvass.itemconfig(self.id, text=self.string_score)
...
score = Score(canvas)                           # 4
textend = Game_end(canvas)                      # 2
paddle = Paddle(canvas, 'blue')
# 2 добавим новую переменную textend
ball = Ball(canvas, paddle, textend, score, 'red')
